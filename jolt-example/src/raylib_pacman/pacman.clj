...
;; tile-centre movement rule are all his, and this file is a port of it onto jolt.ffi, by way of the
;; `pacman` example in jlt-commons/raylib-jlt, which is itself a port of it.
...
(ns raylib-pacman.pacman
  "Pac-Man on raylib, called from jolt over jolt.ffi.

  Run it with `jolt -M:pacman`, or `bb pacman`. Arrows or WASD steer, ENTER
...
(defn -main
  ...
  example runnable unattended.
    ...
    jolt -M:pacman             play it
    jolt -M:pacman 10          quit after ten seconds
    jolt -M:pacman 3 out.png 75   quit after three, capture frame 75
